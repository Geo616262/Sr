...
const handler = async (m, {conn, usedPrefix, text}) => {
  ...
  const textquien = `*Etiqueta a alguien*\n\n* Ejemplo:*\n${usedPrefix}suit @${global.suittag}`;
  if (!m.mentionedJid[0]) return m.reply(textquien, m.chat, {mentions: conn.parseMention(textquien)});
  if (Object.values(conn.suit).find((room) => room.id.startsWith('suit') && [room.p, room.p2].includes(m.mentionedJid[0]))) throw `*[❗] La persona esta jugando`;
  const id = 'suit_' + new Date() * 1;
  const caption = `🎮 GAMES PVP GAMES 🎮\n\n @${m.sender.split`@`[0]} Desafía a @${m.mentionedJid[0].split`@`[0]} A piedra, papel o tijera\n Escribe "aceptar" para aceptar\nEscribe "rechazar" para rechazar\nrespondiendo al mensaje`;
  ...
  conn.suit[id] = {
    ...
    status: 'wait',
    waktu: setTimeout(() => {
      if (conn.suit[id]) conn.reply(m.chat, `[ ⏳ ] Tiempo finalizado, se cancela el juego`, m);
